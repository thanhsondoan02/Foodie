...
const LoginFragmentShipper = ({ closeLoginFragment, isLoginBoxOpen,
  ...
  const onSubmitClick = async (e) => {
    ...
    if (submitCheck(formValue.valueLogin, checkValueLoginError, setValueLoginError)
      ...
      try {
        ...
        if (response.data.EC === 0 && response.data.DT.groupWithRoles.id === 3) {
          closeLoginFragment();
          resetFragment();
          setIsValidAdmin(true);
          localStorage.setItem('shipper_token', response.data.DT.access_token);
          validateToken();
        } else {
          let errorMessage = response.data.EM;
          if (response.data.EC === 0) {
            errorMessage = "You are not a shipper.";
          }
...
export default LoginFragmentShipper;